...
def download(file_name):
    print('启动下载进程[%d]'%getpid())
    print('开始下载%s'%file_name)
    time_to_download = randint(1,4)
    sleep(time_to_download)
    print('%s下载完成！共消耗%d秒' % (file_name, time_to_download))


def main():
    start = time()
    p1 = Process(target=download, args=('学习手册',))
    p1.start()
    p2 = Process(target=download, args=('日本攻略',))
    p2.start()
    p1.join()
    p2.join()
    end = time()
    print('总共耗时等于%d' % (end - start))
...
def main():
    q = Queue()
    for i in range(1,13):  # 制作一个队列含有12个数字
        q.put(i)
...
#threading.Condition()， 可以把Condiftion理解为一把高级的琐，它提供了比Lock, RLock更高级的功能，允许我们能够控制复杂的线程同步问题。
def Seeker(cond, name):
    time.sleep(2)
    cond.acquire()  #获得锁
    print('%s :我已经把眼睛蒙上了！'% name)
    cond.notify()  #通知hider（1）
    cond.wait()    #挂起，等着被通知（2）
    for i in range(3):
...
def Hider(cond, name):
    ...
    cond.notify()  #藏好了，通知（2）
    cond.wait()    #挂起，等着被通知（3）
    cond.release()
